...
plot [x_min:x_max] g(x) ls 3 t "g(x)",\
 "G.txt" i 0 w l ls 2 t "G(x); N=11, {/Symbol a} = 0.0",\
 "pkt.txt" i 0 ls 1 t "g(x_i)"

# 2. APROKSYMACJA: N = 11, alpha = 0.5
set output "app2.eps"
plot [x_min:x_max] "pkt.txt" i 1 ls 1 t "g(x_i)",\
 "G.txt" i 1 w l ls 2 t "G(x); N=11, {/Symbol a} = 0.5"

# 3. APROKSYMACJA: N = 101, alpha = 0.5
set output "app3.eps"
plot [x_min:x_max] "pkt.txt" i 2 ls 1 t "g(x_i)",\
 "G.txt" i 2 w l ls 2 t "G(x); N=101, {/Symbol a} = 0.5"
